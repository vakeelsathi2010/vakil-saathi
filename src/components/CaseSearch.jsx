...
export default function CaseSearch({ embedded = false }) {
  ...
  function exportCases(items) {
    const rows = [['Case Number', 'Case Name', 'Client', 'Type', 'Status', 'Next Date', 'Court'], ...items.map(item => [item.case_number, item.case_title, item.clients?.full_name, item.case_type, item.status, item.next_date || '', item.court_name])]
    const blob = new Blob([rows.map(row => row.map(csvEscape).join(',')).join('\n')], { type: 'text/csv;charset=utf-8' })
    const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'vakil-saathi-cases.csv'; link.click(); URL.revokeObjectURL(link.href)
  }
